            ...
            request(app)
              ...
              .end((err, res) => {
                Availability.findAll({
                  ...
                  assert.equal(availabilities[0].userId, userId);
          ...
          request(app)
            ...
            .end((err, res) => {
              Comment.findAll({
                ...
                assert.equal(comments[0].scheduleId, scheduleId);
                assert.equal(comments[0].userId, userId);
          ...
          request(app)
            ...
            .end((err, res) => {
              Schedule.findByPk(scheduleId).then((schedule) => {
                assert.equal(schedule.scheduleName, 'テスト更新予定2');
                assert.equal(schedule.memo, 'テスト更新メモ2');
              });
              Candidate.findAll({
                where: { scheduleId: scheduleId },
